 ...
 *
 * @author Ji Peng (jipeng@btte.com)
 *
 */
...
var doCombine = function (callback) {
  ...
  var name = path.slice((destination + '/js').length + 1, -3);
  var combineOptions = {
    'baseUrl': destination + '/js',
    'name': name,
    'out': path,
...
var walkJavascripts = function (path) {
  ...
  filesList.forEach(function (currentFile) {
    ...
    if (fs.statSync(tPath).isDirectory()) {
      ...
      if (ext == 'js') {
        if (tPath.indexOf(destination + '/js/page') != -1) {
...
exports.run = function (callback) {
  ...
  try {
    utils.log('正在压缩通用脚本...');
    walkJavascripts(destination + '/js');
